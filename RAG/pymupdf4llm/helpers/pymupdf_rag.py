...
def to_markdown(
    ...
    *,
    ...
    graphics_limit=None,
    threshold=0.5, # !CHANGE! [vector graphic] threshold for vector graphics intersection ratio
) -> str:
    ...
    IMG_PATH = image_path
    THRESHOLD = threshold # !CHANGE! [vector graphic] threshold for vector graphics intersection ratio 
    ...
    def write_text(
        ...
        splitflag=False # !CHANGE! [element splitting] flag for spliting items to list not plain text
    ) -> string:
        ...
        if clip is None:
            clip = textpage.rect
        out_string = ""
        out_tables = [] # !CHANGE! [element splitting]
        out_images = [] # !CHANGE! [element splitting]
        ...
        for lrect, spans in nlines:
            ...
            for i, _ in sorted(
                ...
                # !CHANGE! [element splitting]
                if not splitflag:
                    out_string += "\n" + tabs[i].to_markdown(clean=False) + "\n"
                else:
                    out_tables.append(tabs[i].to_markdown(clean=False))
            ...
            for i, temp_rect in sorted(
                ...
                if force_text:
                    ...
                    if not is_white(img_txt):
                        if not splitflag: # !CHANGE! [element splitting]
                            out_string += img_txt
                        else:
                            out_images.append(img_txt)
                del img_rects[i]
            ...
            if ( # check if we need another line break
                ...
                # !CHANGE! [element positioning] distinguish between line spacing and paragraph spacing
                alpha = 4 if prev_lrect and int((lrect.y0 - prev_lrect.y1)/lrect.height) > 2 else 0 
                out_string += "\n" * max(1, 1+alpha)
            prev_lrect = lrect
        ...
        if not splitflag:
            ...
            out_string = out_string.replace(" \n", "\n").replace("  ", " ").replace("\n\n\n", "\n").replace("\n\n", "\n")
            return out_string, out_tables, out_images
    ...
    # !CHANGE! [vector graphic] Calculate the ratio of rect1 area to rect_i of each element in the rect_list. 
    def intersects_ratio(rect1:fitz.Rect, rect_list:List[fitz.Rect], threshold):
        ratio = 0
        for rect2 in rect_list:
            rect_i = rect1 & rect2
            if not rect_i.is_empty:
                ratio_ = rect_i.get_area() / rect1.get_area()
                if ratio < ratio_: ratio = ratio_
                if ratio > threshold: return ratio
        return ratio
 
    def output_tables(tabs, text_rect, tab_rects, listflag=False):
        """Output tables above a text rectangle."""
        this_md = [] # markdown string for table content # !CHANGE! [element splitting]
        if text_rect is not None:  # select tables above the text block
            for i, trect in sorted(
                ...
                this_md.append(tabs[i].to_markdown(clean=False)) # !CHANGE! [element splitting]
            ...
            for i, trect in sorted(
                ...
                this_md.append(tabs[i].to_markdown(clean=False))
                del tab_rects[i]  # do not touch this table twice

        if not listflag: # !CHANGE! [element splitting]
            this_md = '\n'.join(this_md)
        return this_md
    ...
    # !CHANGE! [vector graphic] Check if it is a text box or not
    def is_text_block(page, rect, threshold=0.3, fontsize=11):
        # blocks = [abs((p[2]-p[0])*(p[3]-p[1]))  for p in page.get_text("blocks", clip=rect)]
        blocks = page.get_text("dict", clip=rect).setdefault('blocks', [])
        spans = [span for block in blocks for line in block.setdefault('lines', []) for span in line.setdefault('spans', [])]
        if len(spans) <= 0: 
            return False
        else:
            blocks = [abs((s['bbox'][2]-s['bbox'][0])*(s['bbox'][3]-s['bbox'][1]))  for s in spans]
            size = max([s['size'] for s in spans])
            return sum(blocks) / rect.get_area() > threshold or size > fontsize
    ...
    def get_page_output(doc, pno, margins, textflags, splitflag=False): # !CHANGE! [element splitting] add "splitflag" parameter
        """Process one page.
        ...
        for bbox in refine_boxes(page.cluster_drawings(drawings=paths)):
            # !CHANGE! [vector graphic] Add image confirmation procedure due to "cluster_drawings" function characteristics
            # !CHANGE! [vector graphic] 
            if is_significant(bbox, paths) and intersects_ratio(bbox, tab_rects0, THRESHOLD) < THRESHOLD and not is_text_block(page, bbox)\
                and intersects_ratio(bbox, [page_clip], 0.5) < 0.5:
        ...
        vg_clusters0 += [fitz.Rect(i["bbox"]) for i in img_info if intersects_ratio(fitz.Rect(i["bbox"]), [page_clip], 0.5) < 0.5] #### some pages all interpreted like image even it is not
        ...
        # !CHANGE! [element positioning] There is a slight y value error in the text box detection with fitz
        def compare_irects(rect1, rect2, tolerance=10):
            # y1 값의 차이가 10 미만이면 x1 값으로 비교
            if abs(rect1.y1 - rect2.y1) < tolerance:
                return rect1.x1 - rect2.x1
            # 그렇지 않으면 y1 값으로 비교
            return rect1.y1 - rect2.y1

        text_rects = sorted(text_rects, key=cmp_to_key(compare_irects))
        # !CHANGE! ====================================================
        ...
        # !CHANGE! [element splitting]
            ...
            table_strings, image_strings = [], []
            for text_rect in text_rects:
                # output tables above this block of text
                table_strings += output_tables(tabs, text_rect, tab_rects, True)
                image_strings.append(output_images(page, textpage, text_rect, vg_clusters)) 

                # output text inside this rectangle
                out_string, out_tables, out_images =  write_text(
                    page,
                    textpage,
                    text_rect,
                    tabs=tabs,
                ...
                table_strings += out_tables
                image_strings += out_images
                md_string += out_string

            md_string = md_string.replace(" ,", ",").replace("-\n", "")
            # write any remaining tables and images      +
            table_strings += output_tables(tabs, None, tab_rects, True)
            image_strings += output_images(page, textpage, None, vg_clusters)
            while md_string.startswith("\n"):
                md_string = md_string[1:]
            return (md_string, table_strings, image_strings), images, tables, graphics
        # !CHANGE! ===============================================
    ...
    for pno in pages:
        ...
        else: # !CHANGE! [element splitting]
            (page_output, table_strings, image_strings), images, tables, graphics = get_page_output(
                doc, pno, margins, textflags, True
            )
            ...
            document_output.append(
                {
                    ...
                    "toc_items": page_tocs,
                    "tables": tables,
                    "tables_s": table_strings,
                    "images": images,
                    "images_s": image_strings,
                    "graphics": graphics,
                    "text": page_output,
                }
            )
