...
remove_chars = dict.fromkeys(range(32))  # 0x00에서 0x1F까지의 문자
remove_chars.update({ord('\x7F'): None})  # 0x7F DEL 문자 \xad
remove_chars.update({ord('\xad'): None})  # soft hypon?
    ...
    for bno, b in enumerate(blocks):  # the numbered blocks
        for lno, line in enumerate(b["lines"]):  # the numbered lines
            for sno, s in enumerate(line["spans"]):  # the numered spans
                ...
                s["text"] = remove_control_characters(s['text'])
